...
/**
 * Загрузчик OpenAPI спецификаций из локальных файлов
 */
public class LocalOpenApiSpecLoader {

    private OpenAPI openAPI;

    public LocalOpenApiSpecLoader(File specFile) throws IOException {
        String content = Files.readString(specFile.toPath());
        initializeFromContent(content);
    }

    public LocalOpenApiSpecLoader(String content) {
        initializeFromContent(content);
    }
    ...
    private void initializeFromContent(String content) {
        ...
        if (this.openAPI == null) {
            throw new RuntimeException("Failed to parse OpenAPI spec from content");
        }
    }
    ...
    public List<ApiEndpoint> extractEndpoints() {
        ...
        System.out.println("✅ Extracted " + endpoints.size() + " endpoints from local OpenAPI spec");
        return endpoints;
    }
