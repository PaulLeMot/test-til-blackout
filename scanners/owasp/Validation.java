...
public class Validation implements SecurityScanner {
    ...
    private String createRealAccount(String baseUrl, ScanConfig config, ApiClient apiClient) {
        try {
            ...
            // Используем первый доступный client token вместо хардкода
            String clientToken = getFirstAvailableClientToken(config);
            if (clientToken != null) {
    ...
    // НОВЫЙ МЕТОД: Получение первого доступного client token
    private String getFirstAvailableClientToken(ScanConfig config) {
        ...
        // Ищем client token (исключаем bank token)
        for (Map.Entry<String, String> entry : userTokens.entrySet()) {
            String key = entry.getKey();
            String token = entry.getValue();

            // Пропускаем bank token и специальные ключи
            if (!key.equals("bank") &&
                    !key.equals("bank_token") &&
                    !key.equals("default") &&
                    token != null &&
                    !token.trim().isEmpty()) {
                return token;
        ...
        // Если не нашли client token, используем default token
    ...
    private List<String> getRealAccountIds(String baseUrl, ScanConfig config, ApiClient apiClient) {
        ...
        try {
            ...
            // Если не нашли своих счетов, пробуем межбанковский запрос с bank token
            if (accountIds.isEmpty()) {
                String bankToken = config.getBankToken();
                if (bankToken != null) {
                    headers.put("Authorization", "Bearer " + bankToken);
    ...
    private String getAppropriateAuthToken(ScanConfig config, Operation operation, String path, Map<String, Object> testData) {
        ...
        if (path.equals("/auth/bank-token") ||
            ...
            return config.getBankToken();
        }

        // Client token для ВСЕГО остального - используем первый доступный
        return getFirstAvailableClientToken(config);
    }
